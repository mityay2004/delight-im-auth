...
//todo clear expired tokens from users_confirmation, users_remembered, users_reset, users_throttling
...
class Auth extends UserManager {
    ...
    /**
     * @param PdoDatabase|PdoDsn|\PDO $databaseConnection the database connection to operate on (not used)
     ...
     */
    public function __construct($ipAddress = null, $dbTablePrefix = null, $throttling = null, $sessionResyncInterval = null, $dbSchema = null) {

        $this->CI = get_instance();

        parent::__construct($dbTablePrefix, $dbSchema);
...
//                    $this->throttle([ 'reconfirmPassword', $this->getIpAddress() ], 3, (60 * 60), 4, false);
    ...
    private function createRememberDirective($userId, $duration) {
        ...
        $this->CI->db
                ->insert($this->makeTableName('users_remembered'),
                    [
                        'user' => $userId,
                        'selector' => $selector,
                        'token' => $tokenHashed,
...
//        $this->throttle([ 'confirmEmail', 'token', $token ], 3, (60 * 60), 10);
        ...
        $confirmationData = $this->CI->db
                ->query(
                    'SELECT a.id, a.user_id, a.email AS new_email, a.token, a.expires, b.email AS old_email
                        FROM ' . $this->makeTableName('users_confirmations') . ' AS a
                        JOIN ' . $this->makeTableName('users') . ' AS b
                        ON b.id = a.user_id
                        WHERE a.selector = ?',
                    [ $selector ]
                )
        ...
        if (!empty($confirmationData)) {
            if (\password_verify($token, $confirmationData['token'])) {
                if ($confirmationData['expires'] >= \time()) {
                    ...
                    $this->CI->db
                            ->where('user', $confirmationData['user_id'])
                            ->delete($this->makeTableName('users_resets')
                    );


//todo check for possible duplicate of users (if user's email already exists in database)


                        // mark the email address as verified (and possibly update it to the new address given)
                        $this->CI->db
                                ->where('id', $confirmationData['user_id'])
                                ->update($this->makeTableName('users'),
                        ...
                        $this->CI->db
                                ->where('id', $confirmationData['id'])
                                ->delete($this->makeTableName('users_confirmations'));
...
//            $this->throttle([ 'enumerateUsers', $this->getIpAddress() ], 1, (60 * 60), 75);
            ...
            $existingUsersWithNewEmail = $this->CI->db
                    ...
                    ->count_all_results($this->makeTableName('users'));
...
//        $this->throttle([ 'enumerateUsers', $this->getIpAddress() ], 1, (60 * 60), 75);
    ...
    private function resendConfirmationForColumnValue($columnName, $columnValue, callable $callback) {
        $latestAttempt = $this->CI->db
                ->query(
                    'SELECT user_id, email FROM '
                        . $this->makeTableName('users_confirmations')
                        . ' WHERE ' . $columnName . ' = ? ORDER BY id DESC LIMIT 1 OFFSET 0',
                    [ $columnValue ]
                )
                ->row_array();
...
//                $this->throttle([ 'attemptToLogin', 'username', $username ], 500, (60 * 60 * 24), null, false);
    ...
    private function getOpenPasswordResetRequests($userId) {
        $requests = $this->CI->db
                ->where(['user'=>$userId, 'expires>'=>time()])
                ->count_all_results($this->makeTableName('users_resets'));
        return (int)$requests;
    }
    ...
    private function createPasswordResetRequest($userId, $expiresAfter, callable $callback) {
        ...
        $this->CI->db
                ->insert(
                    $this->makeTableName('users_resets'),
                    [ 'user' => $userId, 'selector' => $selector, 'token' => $tokenHashed, 'expires' => $expiresAt]
                );
...
//        $this->throttle([ 'resetPassword', 'token', $token ], 3, (60 * 60), 10);
        ...
        $resetData = $this->CI->db
                ->query(
                    'SELECT a.id, a.user, a.token, a.expires, b.resettable
                        FROM ' . $this->makeTableName('users_resets') . ' AS a
                        JOIN ' . $this->makeTableName('users') . ' AS b ON b.id = a.user
                        WHERE a.selector = ?',
                    [ $selector ]
                )
                ->row_array();
                ...
                if (\password_verify($token, $resetData['token'])) {
                    if ($resetData['expires'] >= \time()) {
                        ...
                        $this->CI->db
                                ...
                                ->delete($this->makeTableNameComponents('users_resets'));
                    } else {
                        throw new TokenExpiredException();
                    }
                } else {
    ...
    public function setPasswordResetEnabled($enabled) {
        ...
        if ($this->isLoggedIn()) {
            $this->CI->db
                    ...
                    ->update($this->makeTableNameComponents('users'), [
    ...
    public function throttle(array $criteria, $supply, $interval, $burstiness = null, $simulated = null, $cost = null) {
        ...
        if (!$simulated) {
            ...
            $this->CI->db
                    ->where('bucket', $key)
                    ->update($this->makeTableName('users_throttling'), $bucket);
            $affected = (int)$this->CI->db
                    ->where('bucket', $key)
                    ->count_all_results($this->makeTableName('users_throttling'));
            ...
            if ($affected === 0) {
                ...
                $this->CI->db
                        ->insert(
                            $this->makeTableName('users_throttling'),
                            $bucket);
            }
        }
    ...
    private function getRememberDirectiveExpiry() {
        ...
        if ($this->isLoggedIn()) {
            ...
            if (isset($existingSelector)) {
                ...
                $existingExpiry = $this->CI->db
                        ->query(
                            'SELECT expires FROM ' . $this->makeTableName('users_remembered')
                                . ' WHERE selector = ? AND user = ?',
                            [$existingSelector,$this->getUserId()]
                        )
                        ->row_array();
