...
final class Administration extends UserManager 
{
     ...
     */
    public function __construct($dbTablePrefix = null, $dbSchema = null) {
    ...
    public function getRolesForUserById($userId) 
    {
            ...
            return \array_filter(
                    ...
                    function ($each) use ($rolesBitmask) {
                        return ($rolesBitmask['roles_mask'] & $each) === $each;
                    },
    ...
    private function deleteUsersByColumnValue($columnName, $columnValue) {
        return $this->CI->db
                ...
                ->delete($this->makeTableNameComponents('users'));
    }
    ...
    private function modifyRolesForUserByColumnValue($columnName, $columnValue, callable $modification) {
        ...
        $this->CI->db
                ->query(
                    ...
                    [$newRolesBitmask, (int) $userData['id']]
                );
    ...
    private function logInAsUserByColumnValue($columnName, $columnValue) {
        $users = $this->CI->db
                ->query(
                    'SELECT verified, id, email, username, status, roles_mask 
                    FROM ' . $this->makeTableName('users') 
                    . ' WHERE ' . $columnName . ' = ? LIMIT 2 OFFSET 0',
                    [ $columnValue ]
                )
                ->result_array();
